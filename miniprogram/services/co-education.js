 ...
 * ── 一次提交，可删，不可改 ─────────────────────────────────────────────────
 *
 * 园方 2026-08-31 拍板（契约 v0.7）：
 *
 *   **不存草稿。** 教师填完点发布，客户端弹一次确认让人重看一遍，确认后
 *   `POST /moments` 直接写 `s3`。原先的「建草稿 → 自动保存 → 发布」三步连同
 *   `PATCH` 与 `publication` 两条端点一并退役。
 *
 *   **不给撤回，改为可删。** 原先的 `withdrawal`／`restoration` 换成
 *   `DELETE /moments/{id}`：教师对自己写的内容有处置权，口径与朋友圈一致。
 *   删除是物理删除，同一交易内连带解除入册通道与照片引用。
 *
 * 所以 `db_moment.publish_status` 在教师端实际只会看到 `s3`：
 *   `s1` 由拍板宣告**永久空置**（既有旧数据仍可能读到，照常渲染，见 GAPS G69）；
