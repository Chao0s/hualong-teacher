 ...
 * ── 状态是四档不是两档 ─────────────────────────────────────────────────────
 *
 * 未读 / 已读未完成 / 审核中 / 已完成。**「已读未完成」是 F24 加出来的那一档** ——
 * 家长点开了但没做，与家长根本没看到是两件事，教师要用的催办手段完全不同。
 * 已完成的必然读过（打不开就交不了），所以那一档不重复说「已读」。
 * `under_content_check` 为真时那一笔正在微信内容检查里，压过其余判断。
 * 折算在 service 里做一次（`stateLabel`／`stateTone`），页面不判枚举。
 *
 * ── 顶上的三个数字 ─────────────────────────────────────────────────────────
 *
 * 「班级人数」是**目前**班上在园幼儿数（与看板行数同一个集合），不是发布当时的名册
 * 快照。第三格是「未读」—— 原型那一格本来就写着「已读」，F24 之前全库没有落点，
 * 现在有了。取未读而不是已读：未读是教师要去催的那个数。
 *
 * ── 「结束任务」在这一页 ───────────────────────────────────────────────────
 *
    ...
    try {
      ...
      this.setData({
        ...
        metrics: [
          ...
          // 「未读」而不是「审核中」（F24）：未读是教师要去催的那个数 ——
          // 家长根本没看到，催「快去做」是催错了。审核中在行内那一列看得到，
          // 而共用样式的 .metric-row 是三格网格，不为这一页改成四格。
          { value: String(board.summary.unread), label: '未读' },
        ],
        loading: false,
      });
