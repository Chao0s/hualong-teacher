 ...
 *
 * **照片区本轮不接 —— 原因 2026-09-12 更正过。** 原先写的理由是「`GET
 * /children/{child_id}/term-evaluation` 不回 `file_id`（服务端漂移）」。**实测它回**
 * （幼儿 3 得 `[]`）—— 契约的 `TermEvaluation` 本来就声明了这个字段，空的原因是
 * `db_file_ref` 里 `owner_object='db_term_eval'` 一行都没有，不是字段缺失。
 *
 * 真正接不上的在**相册那一侧**：G28（2026-08-01 已决，按 E7）定「相册 = 该幼儿有份的
 * 那些 moment 的全部照片，靠 `db_moment_upload` 筛 moment」，而教师侧**没有一条端点做
 * 这件事** —— `GET /moments` 的 `child_id` 在 teacher 分支从不读（新登记 **G113**，
 * 传 999 这种不存在的幼儿都回 200）。所以按幼儿筛一次都没发生过。
 *
 * 没有数据源就不要渲染它，更不要编一个出来（CLAUDE.md §8）。原型那份按月分组的
 * 假相册因此删掉，入口留着并置灰，写明待接入 —— 等 G113 拍板。
 */
