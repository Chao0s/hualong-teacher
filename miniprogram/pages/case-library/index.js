 ...
 *
 * **2026-09-09 之前这三条筛选里有两条点了不筛**（G84）：服务端的 WHERE 只有
 * `case_grade` 一句，`case_field` 与 `case_area` 收下即丢，五个领域值与五个活动
 * 类型值各回同一批 9 条。页面这一侧一直是对的 —— 参数发出去了，回来的没筛。
 * 探针也没抓到，因为那三条活动类型断言写的是「多选合并 == 两次单选的并集」，
 * 在服务端忽略参数时恒真。现在探针改成逐值钉库里的行。
 */
