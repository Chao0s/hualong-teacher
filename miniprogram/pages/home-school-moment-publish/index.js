/**
 * 发布活动 —— 写入 `POST /moments`、`PATCH /moments/{id}`、`POST …/publication`。
 *
 * 原型把一份填好的示范表单和 8 个幼儿写死在这里；已经删掉。名单现在是本班真实
 * 在园幼儿，自动保存也真的发请求了。
 *
 * ── 没有草稿，改成发布前确认一次 ───────────────────────────────────────────
 *
 * 园方 2026-08-31 拍板（契约 v0.7）：服务端不存草稿，一次提交即发布。
 * 原型的「保存中…／已自动保存」整块删掉了 —— 那本来也只是 600ms 后换个字，
 * 不发任何请求；现在连要保存的东西都没有了。
 *
 * 取而代之的是**发布前弹一次确认**：`wx.showModal` 把标题与涉及人数摆出来，
 * 让教师在内容发出去之前重看一眼。这是拿一次确认换掉整条草稿链，理由是教师端
 * 的实际场景是当场记录当天活动，跨会话续写的价值低于三条端点与一个自动保存
 * 状态机的成本。
 *
 * **用 `wx.showModal` 而不是同按钮二次点击。** 规格 866 行那条「不调用宿主可能
 * 拦截的原生 confirm」针对的是网页的 `window.confirm`；小程序的 `wx.showModal`
 * 是平台 API，不会被宿主拦掉。
 *
 ...
 *    选了照片也存不进去 —— 这里如实说明，**不往列表里塞假的「新照片 N」**，那正是
 *    原型在做的事。
 *
