...
### 7.13 服务不会热载入：旧服务与活着的长得一模一样

`tools/swagger/server.mjs` **只在启动时读一次模块**。没有热载入。
所以一个服务永远吐它启动那一刻的代码，**你在那之后改的 `pages-view.mjs` 它一个字节都看不到。**

**而新旧两个页面看起来是同一个页面。** 差的只是内容。实测同一天：

```
3830  1,103,583 bytes   ← 改文字之前那一版
3831  1,453,839 bytes   ← 改完之后
```

**用户连报两次「還是沒有文字」，中间还去清了浏览器 cache。清 cache 当然没用 ——
旧的是服务端，不是缓存。** 而页面上没有任何一处说得出「你在看哪个服务、它何时起的」。

**用 `launch api-doc.bat`，别手起。** 它现在会先把 3830–3837 上还活着的收干净、报出收掉的
PID，然后**固定用 3830**（不再「挑第一个空闲的」—— 那会让地址每跑一次漂一格，
而你手上那个旧分页就悄悄指向另一个服务）。

**判据**：`netstat -ano | grep LISTENING | grep :3830` 有且只有一个进程。
多于一个就是有孤儿。八个埠占满时那个 .bat 会直接拒绝启动 —— 那就是孤儿的数量证据。

### 7.14 `.bat` 装不下非 ASCII 注释

cmd 用**主控台编码**读 `.bat`，而本仓的文件是 UTF-8。
中文注释的字节被误读，于是**字符被吃掉、下一行的碎片被当成命令执行**。

同一个缺陷在不同字节对齐下吐出不同的碎片，所以从报错认不出是同一件事：

```
's' is not recognized
'em' is not recognized
'etstat' is not recognized      ← netstat 的 n 被上一行吃掉
```

它还会让启动横幅**重印一遍**（被误读的那一块重跑）。用户看到它很多次，**一直当成无关的主控台杂讯。**

**规矩：`.bat` 里的注释一律 ASCII。** 推算与理由写在 `docs/` 或这里，不写在 `.bat` 里。

**判据**：把那个 .bat 逐行读出来，数含非 ASCII 的行，**应为 0**。
