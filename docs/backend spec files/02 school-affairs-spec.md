...
| 8 | 家园社共育 | Home-School Coeducation | nav_home_school | nav_home_school | NULL | home-school.html |
...
study_id (党建学习ID), 0:k, integer, ui=party.banner|party.study.list
activity_id (党建活动ID), 0:k, integer, ui=party.activity.list
brand_id (品牌建设ID), 0:k, integer, ui=party.brand.list

rel_count (关系数量) = 5
rel_db (关联表) = db_teacher, db_school, db_party_study, db_party_activity, db_party_brand
rel_map (关系字段) = db_party_home{teacher_id}<->db_teacher{teacher_id}; db_party_home{school_id}<->db_school{school_id}; db_party_home{study_id}<->db_party_study{study_id}; db_party_home{activity_id}<->db_party_activity{activity_id}; db_party_home{brand_id}<->db_party_brand{brand_id}
persist (是否持久化) = 0
...
study_type (学习类型), 1:1, t1=policy(政策文件)|t2=learning(学习材料)|t3=system(制度文件), ui=party.study.type
study_content (正文), 0:1, max_len=2000, ui=party_study_detail.content
...
video_links (外部视频链接), 0:k, json_array<{title:max_len=100,url:https}>, ui=party_study_detail.video
...
method (方法):
list = FILTER(school_id=current_school_id, study_status=s3) ORDER BY published_at DESC, study_id DESC CURSOR(published_at,study_id)
home_list = list LIMIT 3
banner_list = list LIMIT 3
list_summary = PREFIX(study_content, 100), computed, persist=0
...
activity_content (正文), 0:1, max_len=2000, ui=party_activity_detail.content
...
method (方法):
list = FILTER(school_id=current_school_id, activity_status=s3) ORDER BY activity_at DESC, activity_id DESC CURSOR(activity_at,activity_id)
home_list = list LIMIT 3
list_summary = PREFIX(activity_content, 100), computed, persist=0
...
brand_content (正文), 0:1, max_len=2000, ui=party_brand_detail.content
...
method (方法):
list = FILTER(school_id=current_school_id, brand_status=s3) ORDER BY published_at DESC, brand_id DESC CURSOR(published_at,brand_id)
home_list = list LIMIT 3
list_summary = PREFIX(brand_content, 100), computed, persist=0
click = return brand_id


[EMPTY_STATE]

IF study_count=0, show_banner=0
...
search = NONE
filter = NONE
pagination = cursor + load_more
cursor_rule = each list uses its business date plus primary key; offset pagination is forbidden


[FILE_REF_RULE]

owner_object = db_party_study|db_party_activity|db_party_brand
usage_key = main_file|inline_media|download
study_main_file = REQUIRE at least 1 db_file_ref usage_key=main_file before direct publish
activity_file = OPTIONAL
brand_file = OPTIONAL


[CONTENT_ACCESS_EVENT_RULE]

detail_open_success = INSERT db_content_access_event(event_type=e3 viewed, actor_type=a1 teacher, selected party target)
file_response_success = INSERT db_content_access_event(event_type=e4 downloaded, actor_type=a1 teacher, selected party target, file_id)
repeat_rule = every successful user action inserts a new event
exclude = failed request|admin preview|copy external link
metric = aggregate event rows; db_content_metric remains persist=0


[EXTERNAL_LINK_RULE]

video_links = JSON array of {title,url}; title required max 100; url required HTTPS
teacher_action = copy URL only
teacher_copy_hint = 请复制链接到浏览器中打开
inline_webview = FORBIDDEN
backend_media_upload = FORBIDDEN
