...
| 15 | 资源与案例不做「撤回」按钮。教师端只有保存草稿与提交审核两个动作；提交后由 PC 端两道关处置，被驳回（`s4`）的直接改、直接重交，不经草稿。已通过（`s3`）要停止流通是管理者的动作。 | 后端 `DECISIONS.md` F27（覆写 F6 的作者撤回分支）；`db/GAPS.md` G74（契约里两条 `withdrawal` 端点待删） |
