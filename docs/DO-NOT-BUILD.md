...
| 15 | 资源与案例不做「撤回」按钮。教师端只有保存草稿与提交审核两个动作；提交后由 PC 端两道关处置，被驳回（`s4`）的直接改、直接重交，不经草稿。已通过（`s3`）要停止流通是管理者的动作。 | 后端 `DECISIONS.md` F27（覆写 F6 的作者撤回分支）；`db/GAPS.md` G74（**已修**，2026-09-09 两条 `withdrawal` 端点已从契约、登记表与服务端删除，见契约 §15 v0.24） |
| 16 | 教师寄语提交后不做「重新编辑」、修改、撤回或删除。一名幼儿一学期只提交一次，成功写入即永久只读；正文在提交前只存在于客户端。契约里因此没有本对象的 `PATCH`、改写 `PUT` 或 `DELETE`。 | 后端 `DECISIONS.md` F16 第二点（删掉了 E1 的编辑与接管分支）；`db_teacher_message` 无状态列、无 revision、无修改历史；契约 §15 v0.22 |
